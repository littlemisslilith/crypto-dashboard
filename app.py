...
df["MACD_diff"] = MACD(close=df["close"].squeeze()).macd_diff()
df["EMA50"] = EMAIndicator(close=df["close"].squeeze(), window=50).ema_indicator()
df["EMA200"] = EMAIndicator(close=df["close"].squeeze(), window=200).ema_indicator()
...
# --- GBM 24H FORECAST (with upgraded formula) ---
